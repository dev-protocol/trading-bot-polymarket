...
import { privateKey, dryRun, clobHost, polyRpcUrl, startingCash, pairOrderLimit, pauseWaitSec, limitPauseCount, rebalanceSize, rebalanceOrderSize, logToFile, autoRedeemDelaySec, } from "./config.js";
...
import { defaultDetectorState, resetDetectorState, bookTo5Deep, bestBidAsk, detect, } from "./detector.js";
import { walletAddressAndBalance, approveAllowance } from "./allowance.js";
import { defaultExecutor, loadUserAuth } from "./clob_client.js";
import { buildMarketSubscribe, buildUserSubscribe, runMarketWssUntil, runUserWssUntil, } from "./wss_listener.js";
import { getOrderSize, getRebalanceOrderSize, liquidityOkForPair, placeUpOrder, placeDownOrder, cancelOrder, } from "./pair_orders.js";
...
function canPlace(executor, yes, no) {
    return (!!executor || dryRun()) && yes.length > 0 && no.length > 0;
}
async function maybeRebalance(state, portfolio, executor) {
    const rebalanceSz = rebalanceSize();
    if (rebalanceSz <= 0)
        return true;
    if (!portfolio || !executor)
        return true;
    ...
    for (let r = 0; r < maxRounds; r++) {
        ...
        if (Math.abs(imbalance) < rebalanceSz)
            return true;
        ...
        if (imbalance > 0) {
            ...
            const { ok, status, orderId } = await placeDownOrder(executor, priceDown, noAssetId, amount);
            if (!ok)
                return false;
            if (status === "matched") {
                portfolio.applyImmediateFill(orderId, conditionId, "DOWN", "BUY", amount, priceDown);
                logInfo(`REBALANCE filled ${amount.toFixed(1)} — checking again`);
            }
        }
        else {
            const { ok, status, orderId } = await placeUpOrder(executor, bestAsk, yesAssetId, amount);
            if (!ok)
                return false;
            if (status === "matched") {
                portfolio.applyImmediateFill(orderId, conditionId, "UP", "BUY", amount, bestAsk);
                logInfo(`REBALANCE filled ${amount.toFixed(1)} — checking again`);
            }
        }
...
async function runPauseThenResume(state, portfolio, executor) {
    ...
    if (limit > 0 && state.pauseCount >= limit) {
        logInfo(`STOP Reached pause limit (${state.pauseCount}/${limit}) — no more pair orders`);
    }
    else {
...
async function main() {
    ...
    if (privateKey())
        console.log("PRIVATE_KEY loaded");
    else
        console.log("PRIVATE_KEY not set — DRY RUN only");
    ...
    if (placeReal && privateKey()) {
        ...
        try {
            const { address, balance } = await walletAddressAndBalance(privateKey(), rpcUrl);
    ...
    logInfo(`PAIR_ORDER_LIMIT=${pairOrderLimit()}  PAUSE_WAIT_SEC=${pauseWaitSec()}  LIMIT_PAUSE_COUNT=${limitPauseCount()}  REBALANCE_SIZE=${rebalanceSize()}  REBALANCE_ORDER_SIZE=${rebalanceOrderSize() ?? "default"}`);
    const unredeemed = unredeemedMarkets();
    if (unredeemed.length > 0) {
        logInfo(`STARTUP would redeem ${unredeemed.length} unredeemed market(s) (onchain not wired)`);
    }
    ...
    for (;;) {
        ...
        if (prevSlug != null && prevCid != null && autoRedeemDelaySec() > 0) {
            logInfo(`AUTO_REDEEM would schedule in ${autoRedeemDelaySec()}s for previous market (onchain not wired)`);
            logger.info(`AUTO_REDEEM would schedule for previous market (onchain not wired)`);
        }
        prevSlug = null;
        prevCid = null;
        const windowEnd = market.windowEndSec ?? getWindowEndTs15m();
        const nowSec = Math.floor(Date.now() / 1000);
        const runForSecs = Math.max(0, windowEnd - nowSec - SWITCH_BUFFER_SEC);
        ...
        logInfo(`WSS market  asset_ids: ${assetIds[0].slice(0, 20)}.. ${assetIds[1].slice(0, 20)}..`);
        ...
        const onBook = (eventType, ev) => {
            if (eventType !== "book")
                return;
            const assetId = ev.asset_id ?? "";
            if (assetId !== state.yesAssetId)
                return;
            const bids = ev.bids ?? [];
            const asks = ev.asks ?? [];
            ...
            const ignoreSignal = (direction === "rise" && bestAsk < 0.5) ||
            ...
            if (direction != null &&
                ...
                const placeUp = async () => {
                    ...
                    const result = placeReal
                        ? await placeUpOrder(executor, bestAsk, state.yesAssetId, sizeUp)
                        : {
                            ok: true,
                            status: "live",
                            orderId: "dry-run-up",
                    ...
                    if (!result.ok) {
                        ...
                        logInfo(`ORDER UP placed at ${bestAsk.toFixed(3)} size=${sizeUp} status=${result.status}`);
                        if (portfolio) {
                            if (result.status === "matched")
                                portfolio.applyImmediateFill(result.orderId, state.conditionId, "UP", "BUY", sizeUp, bestAsk);
                            else
                                portfolio.registerOrder(result.orderId, state.conditionId, "UP", "BUY", sizeUp, bestAsk);
                        }
                        if (result.status === "matched") {
                            const sizeDown = Math.min(orderSz, bestBidSize);
                            const downPrice = Math.max(1.0 - bestAsk - 0.01, 0.01);
                            const followResult = placeReal
                                ? await placeDownOrder(executor, downPrice, state.noAssetId, sizeDown)
                                : {
                                    ok: true,
                                    status: "matched",
                            ...
                            if (!placeReal)
                                logInfo(`DRY RUN would place DOWN at ${downPrice.toFixed(3)} size=${sizeDown}`);
                            if (followResult.ok && followResult.orderId) {
                                logInfo(`ORDER DOWN follow placed size=${sizeDown} status=${followResult.status} order_down_id=${followResult.orderId.slice(0, 24)}..`);
                                if (portfolio &&
                                    followResult.status === "matched")
                                    portfolio.applyImmediateFill(followResult.orderId, state.conditionId, "DOWN", "BUY", sizeDown, downPrice);
                            }
                            if (state.pairOrdersPlaced >= pairLimit) {
                                state.pauseCount++;
                                state.pausePlacePairOrder = true;
                                const limitStr = limitPause === 0 ? "∞" : String(limitPause);
                                logInfo(`PAUSE ${state.pauseCount}/${limitStr} ${state.pairOrdersPlaced} pair orders — waiting ${pauseWait}s`);
                                runPauseThenResume(state, portfolio, executor);
                            }
                        }
                        else if ((result.status === "live" || result.status === "delayed") &&
                            result.orderId) {
                            if (placeReal)
                                await cancelOrder(executor, result.orderId);
                            if (portfolio)
                                portfolio.unregisterOrder(result.orderId);
                            logInfo(`ORDER UP cancelled order_id=${result.orderId.slice(0, 24)}..`);
                        }
                        else if (!placeReal &&
                ...
                const placeDown = async () => {
                    ...
                    const sizeDown = Math.min(orderSz, bestBidSize);
                    const result = placeReal
                        ? await placeDownOrder(executor, bestAskDown, state.noAssetId, sizeDown)
                        : {
                            ok: true,
                            status: "live",
                            orderId: "dry-run-down",
                    ...
                    if (!placeReal)
                        logInfo(`DRY RUN would place DOWN at ${bestAskDown.toFixed(3)} size=${sizeDown}`);
                    if (result.status === "live" || result.status === "delayed")
                        state.pairOrdersPlaced = Math.max(0, state.pairOrdersPlaced - 1);
                    if (!result.ok) {
                        ...
                        logInfo(`ORDER DOWN placed at ${bestAskDown.toFixed(3)} size=${sizeDown} status=${result.status}`);
                        if (portfolio) {
                            if (result.status === "matched")
                                portfolio.applyImmediateFill(result.orderId, state.conditionId, "DOWN", "BUY", sizeDown, bestAskDown);
                            else
                                portfolio.registerOrder(result.orderId, state.conditionId, "DOWN", "BUY", sizeDown, bestAskDown);
                        }
                        if (result.status === "matched") {
                            const sizeUp = Math.min(orderSz, bestAskSize);
                            const upPrice = Math.max(bestBid - 0.01, 0.01);
                            const followResult = placeReal
                                ? await placeUpOrder(executor, upPrice, state.yesAssetId, sizeUp)
                                : {
                                    ok: true,
                                    status: "matched",
                                    orderId: "dry-run-up",
                            ...
                            if (!placeReal)
                                logInfo(`DRY RUN would place UP at ${upPrice.toFixed(3)} size=${sizeUp}`);
                            if (followResult.ok && followResult.orderId) {
                                logInfo(`ORDER UP follow placed size=${sizeUp} status=${followResult.status} order_up_id=${followResult.orderId.slice(0, 24)}..`);
                                if (portfolio &&
                                    followResult.status === "matched")
                                    portfolio.applyImmediateFill(followResult.orderId, state.conditionId, "UP", "BUY", sizeUp, upPrice);
                            }
                            if (state.pairOrdersPlaced >= pairLimit) {
                                state.pauseCount++;
                                state.pausePlacePairOrder = true;
                            ...
                            if (portfolio)
                                portfolio.unregisterOrder(result.orderId);
                            logInfo(`ORDER DOWN cancelled order_id=${result.orderId.slice(0, 24)}..`);
                        }
                        else if (!placeReal &&
                            (result.status === "live" || result.status === "delayed") &&
                            result.orderId) {
        ...
        const onUser = (eventType, ev) => {
            if (eventType !== "order" || !portfolio)
                return;
            const orderId = (ev.id ?? ev.order_id ?? "").trim();
            if (!orderId)
                return;
            const cid = (ev.market ?? "").trim();
            const outcome = ((ev.outcome ?? "").trim().toUpperCase().replace("YES", "UP").replace("NO", "DOWN"));
            const side = (ev.side ?? "BUY").trim().toUpperCase();
            const price = parseFloat(String(ev.price ?? 0)) || 0;
            const msgType = ev.type ?? "";
            if (msgType === "PLACEMENT") {
                const size = parseFloat(String(ev.original_size ?? ev.size ?? 0)) || 0;
            ...
            if (msgType === "UPDATE") {
                ...
                const sizeMatched = parseFloat(String(ev.size_matched ?? 0)) || 0;
                portfolio.onOrderUpdate(orderId, sizeMatched, cidUse, outcomeUse, side, priceUse);
        ...
        const userSub = userAuth
            ...
            : null;
        if (!userSub)
            logInfo("User WSS skipped (auth/auth.json not found)");
        await Promise.all([
