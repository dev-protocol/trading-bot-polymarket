...
function minApprovedAmount(): ethers.BigNumber {
  return ethers.constants.MaxUint256.div(2).add(1);
}

export async function walletAddressAndBalance(
  privateKey: string,
  rpcUrl: string
): Promise<{ address: string; balance: ethers.BigNumber }> {
...
async function checkCtfApprovedForAll(
  provider: ethers.providers.Provider,
  account: string,
  ctf: string,
  operator: string
...
export async function approveAllowance(
  ...
  const allowanceCtf = await checkUsdcAllowance(
    provider,
    from,
    usdc,
    ctfAddr
  ...
  const ctfOk = await checkCtfApprovedForAll(
    provider,
    from,
    ctfAddr,
    ctfExchangeAddr
  ...
  if (!ctfOk) {
    ...
    console.log(
      "CTF setApprovalForAll(CTF_EXCHANGE) tx:",
  ...
  if (includeNegRisk) {
    ...
    const allowNegCtf = await checkUsdcAllowance(
      provider,
      from,
      usdc,
      negCtf
    ...
    const ctfNegOk = await checkCtfApprovedForAll(
      provider,
      from,
      ctfAddr,
      negCtf
    ...
    if (!ctfNegOk) {
      ...
      console.log(
        "CTF setApprovalForAll(NEG_RISK_CTF_EXCHANGE) tx:",
        r4.transactionHash
      );
    }

    const allowAdapter = await checkUsdcAllowance(
      provider,
      from,
      usdc,
      negAdapter
    ...
    const ctfAdapterOk = await checkCtfApprovedForAll(
      provider,
      from,
      ctfAddr,
      negAdapter
    ...
    if (!ctfAdapterOk) {
      ...
      console.log(
        "CTF setApprovalForAll(NEG_RISK_ADAPTER) tx:",
        r6.transactionHash
      );
    }
  }
