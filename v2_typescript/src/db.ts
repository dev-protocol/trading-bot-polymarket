...
function loadMarkets(): MarketEntry[] {
  ...
  try {
    ...
    const data = JSON.parse(raw) as MarketEntry[];
...
function saveMarkets(data: MarketEntry[]): void {
  try {
...
export function addMarketToDb(
  ...
  const idx = data.findIndex(
    (e) => e.condition_id === conditionId || e.slug === slug
  );
  if (idx < 0) {
    data.push({
      slug,
...
export function markMarketRedeemed(conditionId: string): void {
  const data = loadMarkets();
...
export function unredeemedMarkets(): [string, string][] {
  const data = loadMarkets();
  return data
