...
export function defaultDetectorState(): DetectorState {
  return { prevBestBid: null, prevBestAsk: null };
}

export function resetDetectorState(state: DetectorState): void {
...
export interface OrderBook5 {
  bids: PriceLevel[];
  asks: PriceLevel[];
}

export function bestBidAsk(ob: OrderBook5): [number, number, number, number] {
...
export function bookTo5Deep(bids: unknown[], asks: unknown[]): OrderBook5 {
  const DEPTH = 5;
  const parse = (arr: unknown[]): PriceLevel[] => {
...
export function detect(
  ...
  if (prevBestAsk != null) {
    if (bestAsk < prevBestAsk && prevBestAsk - bestAsk > 0.001) direction = "rise";
    else if (bestAsk > prevBestAsk && bestAsk - prevBestAsk > 0.001) direction = "fall";
  }
